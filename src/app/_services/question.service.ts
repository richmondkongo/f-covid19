...
export class QuestionService {
  ...
  public read() {
    ...
    let params = this.G.getParams(this.update, arguments);
		return new Promise(
			(resolve, reject) => {
				this.httpClient.post<any>(this.G.link.question, {numero, libelle, deleted}, this.G.getHttpOptions()).subscribe(
					(res) => {
						resolve(res);
					}, (err) => {
						reject(err);
    ...
    return new Promise(
			(resolve, reject) => {
				this.httpClient.put<any>(this.G.link.question + id + '/', {numero, libelle, deleted}, this.G.getHttpOptions()).subscribe(
					(res) => {
						resolve(res);
					}, (err) => {
						reject(err);
